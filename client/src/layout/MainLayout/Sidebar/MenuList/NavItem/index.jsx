...
const NavItem = ({ item, level }) => {
  ...
  const isSelected = location.pathname.startsWith(item.url); // ✅ Keeps the menu active for sub-routes
  ...
  return (
    <ListItemButton
      ...
      sx={{
        ...
        marginBottom: '5px',
        pl: `${level * 16}px`,
        backgroundColor: isSelected ? theme.palette.primary.main : 'inherit', // Highlight active menu
      }}
      selected={isSelected}
      {...listItemProps}
    >
      <ListItemIcon sx={{ minWidth: 25, color: isSelected ? '#fff' : 'inherit' }}>{itemIcon}</ListItemIcon>
      <ListItemText
        primary={
