...
exports.getEmployeeData = async (req, res) => {
    try {
        ...
        if (user.role !== "admin") {
            // Non-admin users only see their own role data
            whereCondition = { role: user.role };
            ...
            availableRoles = distinctRoles.map(item => item.role);
            whereCondition = { role: { [Op.in]: availableRoles } };
        }
